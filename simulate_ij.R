...
f_simulate_ij = function(IJ) {
   ...
   if (O3_POD) assign(x = 'POD_PFT_hist_ijd', value = array(NA, dim=c(length(pftname), 24/dt_hr))) # Sadiq, Feb 2019
   ...
   for (ipft in 2:length(pftname)) {
      ...
      if (LAI < 0.01 | PFT_frac[i,j,ipft] < 0.01) {
         #print(paste0('Skipped PFTs due to LAI or PFT fraction: ', ipft))
         #print(paste0('LAI: ', LAI))
         #print(paste0('PFT fraction: ', PFT_frac[i,j,ipft]))
         ...
         for (h in 1:(24/dt_hr)) {
            ...
            if (!O3_POD) POD_prev = 0
               ...
               if (h == 1 & (d == 1 & !continue_flag)) POD_prev = 0
                  ...
                  # Cumulative POD of the last hour of last day:
                  # Date before current date:
                  previous_date = to.yyyymmdd(from.yyyymmdd(current_date) - 1*24)
                  filename = paste0(simulation_dir, 'temp_data/temp_', as.character(previous_date), '/temp_i', i_str, '_j', j_str, '.RData')
                  load(filename)
                  POD_prev = POD_PFT_lasthd_ijd[ipft]
               } else
                  POD_prev = POD_PFT_hist_ijd[ipft,(h-1)]
            }
            ...
            if (simple_radiation_flag) {
               if (cos_SZA > 0) {
                  ...
                  phi_sha = PAR_diff * exp(-0.5 * LAI^0.7) + 0.07 * PAR_beam * (1.1 - 0.1 * LAI) * exp(-cos_SZA)  # PAR_beam corrected to PAR_diff (Tai, Feb 2019)
            ...
            canopy_photosyn = f_canopy_photosyn(c_a=c_a, e_a=e_a,
                                                ...
                                                POD_prev=POD_prev, # Sadiq, Feb 2019
                                                crop_harvest = crop_harvest,
                                                n_day_whole=n_day_whole,
            ...
            output_assign_df = `colnames<-`(rbind.data.frame(
               ...
               c('f_d_PFT_hist_ijd', 'canopy_photosyn$f_d'),
   ...
   if (O3_POD) temp_env$POD_PFT_lasthd_ijd = POD_PFT_hist_ijd[,24/dt_hr] # Sadiq, Feb 2019
