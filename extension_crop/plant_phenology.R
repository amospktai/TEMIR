f_crop_phenology = function(T_10_d, T_min_10_d, T_soil, T2m,
  ...
  if (get_planting_date_option == 'CLM4.5') {
      ...
      # initialize the crop just before planting
  ...
  if (!crop_living_flag && !crop_planting_flag) {
    ...
    if (crop_living_flag) {
        ...
        if (get_GDDmat_method == "custom"){
            # 'prescribed_GDD_mat' is an input at 'input_TEMIR_biogeochem_extension.R'
      ...
      if (GDDmat < GDDrepr || GDDmat < GDDemer || GDDrepr < GDDemer){
        warning("The values of GDDmat or/and GDDrepr or/and GDDemer have some problems, make sure they follow the order of 'GDDmat > GDDrepr > GDDemer'")
      }
    } else {
  ...
  if (crop_living_flag) {
    ...
    # calculating DVI for custume senescence scheme
    if (GDD_T2m < GDDemer) {
    ...
    if (GDD_T2m >= GDDmat || days_since_planting > max_growing_season_length) { 
    ...
    } else if ((GDD_T2m >= GDDrepr && GDD_T2m < GDDmat && crop_leaf_senescence_scheme == 'CLM4.5') ||
               (DVI >= leaf_senescence_DVI && crop_leaf_senescence_scheme == 'custom')) {    # at reproductive stage
      ...
      if (crop_leaf_senescence_scheme == 'CLM4.5') {
          # leaf senescence rate for every timestep during grain fill, unit: gC m^-2 s^-1, leaf_longevity = leaf longevity (yr) from PFT_surf_data.R
          leaf_C_loss_flux = 1/(leaf_longevity * 86400 * 365) * leafC  
          livestem_C_loss_flux = 0
          
      } else if (crop_leaf_senescence_scheme == 'custom') {
          # calibrated from soyFACE data
  ...
  output = list(seedC_to_leafC_flux = seed_C_to_leaf_C_flux, leafC_loss_flux = leaf_C_loss_flux, grainC_loss_flux = grain_C_loss_flux, livestemC_loss_flux = livestem_C_loss_flux, finerootC_loss_flux = fineroot_C_loss_flux,
                ...
                CUO_grainfill = CUO_grain_filling)
